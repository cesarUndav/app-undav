...
const refetchEventos = useCallback(async () => {
    // 🌟 NUEVA LÓGICA PARA VISITANTES (SIN LOGIN)
    if (!usuarioAutenticado) {
        setIsLoading(true);
        setError(null);
        try {
            console.log("📅 [AgendaContext] Modo Visitante: Sincronizando calendario institucional público...");
            
            // 1. Forzamos la petición HTTP pesada a la API PHP aunque no haya token
            await cargarEventosAcademicos();
            
            // 2. Traemos los eventos que no hayan finalizado
            const academicos = listaFuturo();
            
            // 3. Llenamos el estado global (aquí irán los académicos puros)
            setEventosFuturos(academicos);
            
            // 4. Limpiamos lo referente a usuarios logueados
            setEventosPersonalizados([]);
            setIdUsuarioActual(null);
            ultimoUsuarioCargado.current = null;
        } catch (err: any) {
            console.error("❌ Error en Context (Visitante):", err?.message || err);
            setError("No se pudieron cargar los eventos académicos.");
        } finally {
            setIsLoading(false);
        }
        return; // ✋ Cortamos acá. El visitante no necesita verificar SecureStore ni AsyncStorage.
    }

    // 🔒 LÓGICA PARA USUARIOS LOGUEADOS (Queda igual que antes)
    ...
    try {
        console.log("📅 [AgendaContext] Credenciales validadas. Sincronizando datos de estudiante...");
