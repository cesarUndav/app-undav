...
async function obtenerTablasListadornot(): Promise<
...
> {
  ...
  const tablaRegex =
    /<table[^>]*class=["']listadornot["'][^>]*>([\s\S]*?)<\/table>/gi;
  ...
  while ((matchTabla = tablaRegex.exec(html)) !== null) {
    ...
    const labelMatch = tr1.match(
      /<td[^>]*class=["']titulo["'][^>]*>([\s\S]*?)<\/td>/i
    );

    if (!labelMatch) continue;

    let labelLimpio = labelMatch[1].replace(/<[^>]+>/g, '').trim();
    labelLimpio = he.decode(labelLimpio);
      ...
      <LoadingWrapper loading={loadingLinks}>
        <ScrollView contentContainerStyle={styles.listaContainer}>
          ...
          <BotonTexto
            ...
            url="https://undav.edu.ar/index.php?idcateg=323"
            openInsideApp
